CREATE DATABASE  IF NOT EXISTS `apprenticehoursdb` /*!40100 DEFAULT CHARACTER SET utf8 */;
USE `apprenticehoursdb`;
...
CREATE TABLE `monthlysubmissionstbl` (
  ...
  `comment` text,
  PRIMARY KEY (`msid`),
  KEY `persmsfk` (`persmsfk`),
  CONSTRAINT `persmsfk_foreignkey` FOREIGN KEY (`persmsfk`) REFERENCES `personstbl` (`personid`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `wagestbl` (
  ...
  CONSTRAINT `persoccwagefk_foreignkey` FOREIGN KEY (`persoccwagefk`) REFERENCES `personoccupationstbl` (`aoid`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
-- Dump completed on 2021-02-22 20:16:29
